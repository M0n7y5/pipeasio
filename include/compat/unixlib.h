/*
 * Bundled copy of Wine's include/wine/unixlib.h (Wine 11.16, unchanged since
 * 11.10), used only when the installed Wine SDK does not ship it: Debian and
 * Ubuntu's libwine-dev leave it out. The contract it describes (the call
 * tables, __wine_unix_call_dispatcher, __wine_init_unix_call) is what the
 * linked libwinecrt0.a implements, so it must stay in step with Wine's.
 * LGPL-2.1-or-later, see the original notice below.
 */
