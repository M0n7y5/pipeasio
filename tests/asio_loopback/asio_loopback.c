...
static int
run_phase(IPipeASIO *asio, LONG bs, int seconds)
{
    BufferInformation bi[2 * NCH];
    Callbacks         cbs = { cb_swapBuffers, cb_sampleRateChanged, cb_sendNotification,
                              cb_swapBuffersWithTimeInfo };
    LONG              rc;
    int               ch;
    ...
    {
        fprintf(stderr,
                "[loop] FAIL: no signal lock after 10 s "
    ...
    for (int t = 0; t < seconds; t++)
    {
        ...
        Sleep(1000);
        fprintf(stderr,
                "[loop]   t=%d: +%ld cycles, discont=%ld/%ld "
                "bits=%ld/%ld sign=%ld/%ld\n",
                t + 1, (long)(g_cycles - before), (long)g_ch[0].discont, (long)g_ch[1].discont,
                (long)g_ch[0].bit_err, (long)g_ch[1].bit_err, (long)g_ch[0].sign_err,
                (long)g_ch[1].sign_err);
    }
    ...
    for (ch = 0; ch < NCH; ch++)
    {
        ...
        if (s->discont || s->bit_err || s->sign_err)
        {
            fprintf(stderr, "[loop] FAIL ch%d: discont=%ld bit_err=%ld sign_err=%ld\n", ch,
                    (long)s->discont, (long)s->bit_err, (long)s->sign_err);
            fail = 1;
        }
        if (s->latency <= 0)
        {
    ...
    {
        fprintf(stderr,
                "[loop] FAIL: measured RTL %lld vs reported %lld "
    ...
    fprintf(stderr,
            ...
            (long)bs, rtl, (double)rtl / (double)bs, rate > 0 ? 1000.0 * (double)rtl / rate : 0.0,
...
main(void)
{
    ...
    if (rc == 0)
    { /* ASIOTrue = 1, ASIOFalse = 0 on this path */
        ...
        for (int isin = 0; isin <= 1; isin++)
        {
            ...
            if (rc != 0 || ci.sampleType != 19)
            {
                fprintf(stderr, "[loop] FAIL: ch%d %s sampleType=%ld (want 19)\n", ch,
                        isin ? "in" : "out", (long)ci.sampleType);
                asio->lpVtbl->Release(asio);
                CoUninitialize();
                return 1;
            }
