 ...
 *
 * Pins the delivery contract end-to-end in ~1 second:
 ...
 *     finite).
 * If the buffer memory is not shared with the daemon (for example an aliased
 * private memfd the daemon never reads) the consumer sees zeros or garbage
 * and the test fails.
 *
...
static void
prod_process(void *u, struct spa_io_position *pos)
{
    ...
    float           *dst = d->data;
    uint32_t         m   = n;
    if (m * SAMPLE_SZ > d->maxsize)
        m = d->maxsize / SAMPLE_SZ;
    if (dst)
...
static void
cons_process(void *u, struct spa_io_position *pos)
{
    ...
    if (src && m >= 2)
    {
        ...
        for (uint32_t i = 0; i < m; i++)
        {
            ...
            if (src[i] != 0.0f)
                nonzero = 1;
            if (i && src[i] - src[i - 1] != 1.0f)
                ramp = 0;
        }
        if (!finite)
            atomic_fetch_add(&co->bad, 1); /* NaN/inf = the memfd-alias bug */
        else if (!nonzero)
            atomic_fetch_add(&co->silence, 1); /* startup zeros, tolerated */
        else if (ramp)
            atomic_fetch_add(&co->ramp_ok, 1); /* exact ramp delivered */
        else
            atomic_fetch_add(&co->bad, 1); /* finite but corrupt */
    }
    pw_filter_queue_buffer(co->port, b);
}
static const struct pw_filter_events consumer_events = {
    PW_VERSION_FILTER_EVENTS,
    .process = cons_process,
};
...
static void
reg_global(void *u, uint32_t id, uint32_t perm, const char *type, uint32_t ver,
           const struct spa_dict *props)
{
    struct reg *r = u;
    (void)perm;
    (void)ver;
    if (!props || !type || strcmp(type, PW_TYPE_INTERFACE_Port))
        return;
...
static uint32_t
reg_find(struct reg *r, uint32_t node, int is_out)
{
    for (int i = 0; i < r->n; i++)
...
static void *
add_dsp_port(struct pw_filter *f, enum pw_direction dir, const char *name, size_t user_sz)
{
    ...
    const struct spa_pod  *params[] = {
        spa_pod_builder_add_object(
                ...
                SPA_PARAM_BUFFERS_stride, SPA_POD_Int((int)SAMPLE_SZ), SPA_PARAM_BUFFERS_align,
                SPA_POD_Int((int)(BSIZE * SAMPLE_SZ)), SPA_PARAM_BUFFERS_dataType,
                SPA_POD_CHOICE_FLAGS_Int(1 << SPA_DATA_MemFd)),
    };
    return pw_filter_add_port(f, dir, PW_FILTER_PORT_FLAG_MAP_BUFFERS, user_sz, pp, params, 1);
}

static struct pw_properties *
node_props(const char *name, const char *cat)
{
    struct pw_properties *p = pw_properties_new(PW_KEY_NODE_NAME, name, PW_KEY_MEDIA_TYPE, "Audio",
                                                PW_KEY_MEDIA_CATEGORY, cat, PW_KEY_MEDIA_ROLE,
                                                "DSP", PW_KEY_NODE_ALWAYS_PROCESS, "true",
                                                PW_KEY_NODE_GROUP, "delivery.probe", NULL);
...
main(void)
{
    ...
    pr.filter = pw_filter_new_simple(pw_thread_loop_get_loop(tl), "delivery_src",
                                     node_props("delivery_src", "Playback"), &producer_events, &pr);
    ...
    co.filter = pw_filter_new_simple(pw_thread_loop_get_loop(tl), "delivery_sink",
                                     node_props("delivery_sink", "Capture"), &consumer_events, &co);
    ...
    for (int i = 0; i < 100; i++)
    {
        usleep(20 * 1000);
        pw_thread_loop_lock(tl);
        if (prod_node == SPA_ID_INVALID)
            prod_node = pw_filter_get_node_id(pr.filter);
        if (cons_node == SPA_ID_INVALID)
            cons_node = pw_filter_get_node_id(co.filter);
        if (prod_node != SPA_ID_INVALID)
            prod_out = reg_find(&reg, prod_node, 1);
        if (cons_node != SPA_ID_INVALID)
            cons_in = reg_find(&reg, cons_node, 0);
    ...
    pw_thread_loop_lock(tl);
    struct pw_proxy *link = pw_core_create_object(core, "link-factory", PW_TYPE_INTERFACE_Link,
                                                  PW_VERSION_LINK, &lp->dict, 0);
    pw_thread_loop_unlock(tl);
    if (!link)
    {
    ...
    fprintf(stderr,
            "[delivery] producer cycles=%d | consumer seen=%d ramp_ok=%d "
            "silence=%d bad=%d\n",
